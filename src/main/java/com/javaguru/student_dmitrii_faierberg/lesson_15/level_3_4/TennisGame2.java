...
class TennisGame2 implements TennisGame{
    ...
    public String score() {
        String score = "";
        score = getEqualScore(score);
        score = getAdvantage(score);
        score = getWin(score);

        if (score.equals(""))
            score = getDifferentScore(score);

        return score;
    }

    private String getWin(String score) {
    ...
    private String getDifferentScore(String score) {
        int tempScore;
        StringBuilder scoreBuilder = new StringBuilder(score);
        for (int i = 1; i<3; i++) {
            if (i==1) tempScore = P1point;
            else { scoreBuilder.append("-"); tempScore = P2point;}
            switch (tempScore) {
    ...
    private String getEqualScore(String score) {
